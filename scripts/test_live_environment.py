...
def request_bytes(url: str, *, data: bytes | None = None) -> tuple[bytes, str]:
...
def request_json(url: str, *, payload: dict[str, object] | None = None) -> dict[str, object]:
    ...
    body, content_type = request_bytes(url, data=data)
    if content_type != "application/json":
        raise AssertionError(f"Expected JSON from {url}, received {content_type}.")
    return json.loads(body)


def png_dimensions(url: str) -> tuple[int, int]:
    request = Request(url)
...
def wait_for_render(api_url: str, job_id: str, timeout_seconds: int) -> dict[str, object]:
    deadline = time.monotonic() + timeout_seconds
    status_url = urljoin(f"{api_url}/", f"renders/{job_id}")
    last_status = "queued"
...
def run(web_url: str, api_url: str, timeout_seconds: int) -> None:
    ...
    completed = wait_for_render(api_url, job_id, timeout_seconds)
    ...
    dimensions = {png_dimensions(str(url)) for url in output_urls}
...
def main() -> int:
    ...
    try:
        run(args.web_url.rstrip("/"), args.api_url.rstrip("/"), args.timeout_seconds)
